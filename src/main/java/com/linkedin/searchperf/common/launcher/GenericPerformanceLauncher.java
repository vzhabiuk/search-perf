...
import com.linkedin.searchperf.runner.ConcurrentRunner;
import com.linkedin.searchperf.runner.PerfRunnerConfig;
import com.linkedin.searchperf.runner.Runners;
import com.yammer.metrics.reporting.ConsoleReporter;

public class GenericPerformanceLauncher {
  public static final String RESULT_FILE = "performanceResult.txt";
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i : threads) {
      perfRunnerConfig.setNumThreads(i);
      ConcurrentRunner concurrentRunner = Runners.createByType(perfRunnerConfig.getType());
      PerformanceResult result = run(perfRunnerConfig, concurrentRunner);
  ...
  public static List<Integer> extractNumberOfThreads(PropertiesConfiguration config) {
  ...
  private static PerformanceResult run(PerfRunnerConfig senseiRunnerConfig, ConcurrentRunner senseiConcurrentRunner)
      throws UnsupportedEncodingException {
    senseiConcurrentRunner.init(senseiRunnerConfig);
    senseiConcurrentRunner.run();
    senseiConcurrentRunner.shutdown();
  ...
  public static PerformanceResult process(String output, PerfRunnerConfig senseiRunnerConfig) {
  ...
  public static PropertiesConfiguration extractPropertyConfig(String[] args) throws ConfigurationException {
